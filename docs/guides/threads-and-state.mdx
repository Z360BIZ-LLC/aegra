...
- Reading `values`/`interrupts` requires decoding checkpoint state, so the server
  fetches only the channels your paths reference (e.g. just `messages`) in one batched
  query — no N+1.
