...
- **`select`** — supported fields: `thread_id`, `status`, `created_at`, `updated_at`,
  `metadata`, `values`. Omit `select` to get the full thread shape (backward compatible).
- **`extract`** — maps an alias to a JSON path. Paths must start with `values.` or
  `metadata.`, support dot keys and array indexing (`[0]`, `[-1]`), and are capped at 10.
  A path that doesn't resolve yields `null`; a malformed path returns `422`.
- Reading `values` requires decoding checkpoint state, so the server fetches only the
  channels your paths reference (e.g. just `messages`) in one batched query — no N+1.

<Note>
The SDK fields `select=["config" | "context" | "interrupts"]` and filtering by state
`values=...` are not yet supported and return `422`. Sorting by `state_updated_at` also
returns `422` (use `updated_at`).
</Note>
